...
"""B2 long-window FrFNO regression (T=0.06, train Nt=1600, test 1600/3200/6400).
Uses new 1-D-table-backed build_prop_table. Old headline: 17^2=2.521%, 65^2=7.807%, 129^2=9.384%.
"""
import os, sys, time
import numpy as np
import torch
...
TN, BETA, NL_NT = 0.06, 3.0, 1600
NTR = 128; STEPS = 8000; NTEST = 48
RES = [(16, 1600), (64, 3200), (128, 6400)]
OUT = os.path.join(ROOT, 'b2_frfno_regression_result.txt')
L = []
def log(s):
...
OLD = {16: 2.521, 64: 7.807, 128: 9.384}

# ---- training data (Nt=NL_NT=1600) ----
log('=== B2 FrFNO long-window regression (T=%g, Nt_train=%d, NTR=%d, STEPS=%d) ===' % (TN, NL_NT, NTR, STEPS))
...
# ---- test data (each resolution uses its own Nt) ----
...
for Nx, Nt in RES:
    ...
    log('test data @%d^2 (Nt=%d) ready' % (Nx + 1, Nt))

# ---- build table (Nt=NL_NT=1600, shared by all resolutions via 1-D cache) ----
t0 = time.time(); PT = {}
for Nx, _ in RES:
    PT[Nx] = build_prop_table(Nx, Nt=NL_NT)
sync(); t_build = time.time() - t0
log('build_prop_table (3 res, Nt=%d): %.2fs (1-D cached, render per res)' % (NL_NT, t_build))
...
log('Regression check done. Compare "diff" column above against old headline numbers (2.521/7.807/9.384 %).')
log('saved -> %s' % OUT)
