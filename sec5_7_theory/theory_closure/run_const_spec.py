# -*- coding: utf-8 -*-
"""Run nu=1, beta=0 (pure constant-coefficient linear) and measure DST tail.
With nu=1, beta=0, the true solution is exactly E0*u0, so residual should be ~0 (numerical error only).
"""
import os, sys
...
seeds, a, s, xi = zr['seeds'], zr['a'], zr['s'], zr['xi']

Nx = 64
Phi, e = P.get_phi(Nx)
U0 = np.stack([LG.generate_multiscale_initial(Nx, 8, int(sd)) for sd in seeds]).astype(np.float32)
# nu = 1 everywhere (constant coefficient)
NU = np.ones_like(U0)
PT = build_pt_eig(Nx, 3200, P.AD, P.SDG)
Q_gpu = spectral_eigvals_torch(Nx, P.DEV, P.DT)

T = P.TN; Nt = 3200
print(f"Running nu=1, beta=0 constant-coefficient linear solve: Nx={Nx}, Nt={Nt}, n={len(seeds)}", flush=True)
...
print("\n=== nu=1, beta=0 (constant-coefficient linear) DST tail ===")
for K in Ks:
...
print(f"Note: with nu=1,beta=0, u_ref should == E0*u0 exactly, so r should be ~0 (numerical error)")
