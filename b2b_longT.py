...
RES = [(16, 400)] if FAST else [(16, 1600), (64, 3200), (128, 6400)]
METHODS = ['FrFNO', 'FNO', 'PINO', 'PDNO', 'CNO', 'DeepONet', 'UNet']
OUT = os.path.join(ROOT, 'b2b_longT_result.txt')
L = []
...
def main():
    ...
    torch.save({k: nets[k].state_dict() for k in nets}, os.path.join(ROOT, 'b2b_weights.pt'))
    ...
    np.savez(os.path.join(ROOT, 'b2b_metrics.npz'),
