# -*- coding: utf-8 -*-
"""
FrFNO: Fractional Fourier Neural Operator for fractional-order PDE surrogate modeling.
PDE: ^CD_t^a u = -nu(x,y)(-Delta)^s u + nonlinear terms, variable-coefficient log-normal nu
Core components:
  - DualNet: FrFNO and FNO backbone with DynFrac2d fractional coordinate filtering
  - DynFrac2d: alpha-controlled fractional low-pass filtering of coordinate fields
  - SpecConv2d: spectral convolution layer with Fourier multipliers
  - build_prop_table: resolution-independent Mittag-Leffler propagator lookup table
  - build_trainset: GPU batch ground truth generation
  - train: unified training loop with cosine LR schedule
  - generate_multiscale_initial, make_nu, kl_basis: data generation utilities
"""
import sys, os, time
import numpy as np
...
OUT = os.path.join(ROOT, 'uq_compare_result.txt')
L = []
...
RES = [(64, 800, 128 if FAST else 256), (128, 1600, 128),
       (256, 1600, 64 if FAST else 128)]   # (Nx,Nt,Nref); Nref is a power of 2 to preserve Sobol balance
...
# ============ 1-D resolution-independent propagator table (new standard) ============
_1D_TABLE_CACHE = {}
_1D_ZMIN, _1D_ZMAX = 1.0, 2.0e5   # covers lambda^s up to Nx=256, s=0.90 (zmax~1.4e5) with margin

def build_prop_table_1d(na=41, ns=41, Nt=400, M=1600):
    """One-dimensional resolution-independent Mittag-Leffler propagator table.

    Returns (AD, SD, zg, T1) where T1[i,j,m] = g_{alpha_i}(z_m), with z = lambda^s.
    g depends on alpha and z only (NOT on s); the s-slice of T1 is redundant and
    repeated for interface symmetry. Cached globally by (na, ns, Nt, M, T): built
    ONCE, shared by every spatial resolution. New code should use this together with
    prop_at_1d; build_prop_table below is a backward-compatible renderer.
    """
    key = (na, ns, Nt, M, float(T))
    if key in _1D_TABLE_CACHE:
        return _1D_TABLE_CACHE[key]
    AD = np.linspace(0.45, 1.05, na); SD = np.linspace(0.25, 0.90, ns)
    zg = np.logspace(np.log10(_1D_ZMIN), np.log10(_1D_ZMAX), M, dtype=np.float32)
    zt = torch.tensor(zg, device=DEV, dtype=DT)
    T1 = np.zeros((na, ns, M), np.float32)
    for ia, al0 in enumerate(AD):
        al = float(al0)
        b = torch.tensor(_l1_b(al, Nt), device=DEV, dtype=DT)
        dta = (T / Nt) ** al; b0 = b[0]
        hist = torch.empty((Nt, M), device=DEV, dtype=DT); hist[0] = 1.0; v = hist[0]
        for k in range(1, Nt + 1):
            ...
            v = rhs / (b0 + dta * zt)
            if k < Nt: hist[k] = v
        T1[ia] = v.cpu().numpy()[None].repeat(ns, 0)
        del hist
    torch.cuda.empty_cache()
    _1D_TABLE_CACHE[key] = (AD, SD, zg, T1)
    return AD, SD, zg, T1

def prop_at_1d(alpha, s, Q, AD, SD, zg, T1):
    """Online lookup from the 1-D table: bilinear in (alpha,s) + linear in z=Q^s.

    The s-dimension of T1 is redundant (g independent of s), so there is no
    s-interpolation error; s enters only through z = Q^s evaluated at the queried
    value, followed by linear interpolation on the shared z-grid. Q is the
    per-resolution five-point eigenvalue field (n,n).
    """
    na, ns = len(AD), len(SD)
    ia = int(np.clip(np.searchsorted(AD, alpha) - 1, 0, na - 2))
    js = int(np.clip(np.searchsorted(SD, s) - 1, 0, ns - 2))
    ta = float((alpha - AD[ia]) / (AD[ia + 1] - AD[ia]))
    ts = float((s - SD[js]) / (SD[js + 1] - SD[js]))
    z = Q.astype(np.float64) ** s
    zg64 = zg.astype(np.float64)
    def vz(curve):
        return np.interp(z, zg64, curve.astype(np.float64)).astype(np.float32)
    c = [T1[ia, js], T1[ia + 1, js], T1[ia, js + 1], T1[ia + 1, js + 1]]
    v = [vz(cc) for cc in c]
    w = [(1 - ta) * (1 - ts), ta * (1 - ts), (1 - ta) * ts, ta * ts]
    out = np.zeros_like(v[0])
    for ww, vv in zip(w, v): out = out + ww * vv
    return out

# ============ Analytic propagator table / FrFNO features ============
def build_prop_table(Nx, na=41, ns=41, Nt=400):
    """Backward-compatible 2-D table renderer.

    Internally builds the 1-D resolution-independent table ONCE (cached globally via
    build_prop_table_1d), then renders it to the per-resolution (na, ns, n, n) tensor
    for compatibility with all existing callers. Rendering cost is O(na*ns*n^2) in
    z-interpolation (no time recursion), vs O(na*ns*n^2*Nt^2) for the old per-resolution
    recursion. New code should call build_prop_table_1d / prop_at_1d directly to skip
    rendering and avoid the (alpha,s)-grid interpolation entirely.
    """
    AD, SD, zg, T1 = build_prop_table_1d(na, ns, Nt)
    Q = spectral_eigvals(Nx); n = Nx - 1
    PT = np.zeros((na, ns, n, n), np.float32)
    for ia in range(na):
        for js in range(ns):
            PT[ia, js] = prop_at_1d(AD[ia], SD[js], Q, AD, SD, zg, T1)
    return AD, SD, PT
...
def tn_ppf(u, p):  # [0,1] -> truncated-normal inverse CDF
...
def discrete_gauss_1d(p, n, ng=4001):
    """n-point Gauss rule under the truncated-normal measure (Golub-Welsch, discrete Stieltjes)."""
    z = np.linspace(p['lo'], p['hi'], ng); w = tn_pdf(z, p); w = w/w.sum()
    a = np.zeros(n); b = np.zeros(n)
    # Three-term Stieltjes recursion coefficients (discrete inner product)
...
def smolyak_2d(L):
    """2-D Smolyak sparse quadrature; the 1-D rule is the discrete Gauss Q_i (level i uses i+1 nodes); merge near-duplicate nodes."""
    nodes = {}
...
def build_trainset():
    log(f'\n=== Build 17^2 training data, {NTR} fields x 9x9 order grid (fully implicit batched GPU) ===')
...
def infer(net, kind, u0s, nus, aa, ss, Nx, AD, SD, PT):
    """u0s/nus:(nte,H,W); forward the M (a,s) samples one field at a time -> list[nte] of (M,H,W) physics fields"""
...
def solve_points(Nx, Nt, u0, nu, aa, ss, Q, bs=16):
    """Batch-solve one field for a set of (α,s) -> (len,D) and the per-sample wall time"""
...
def main():
    ...
    log(f'  Neural-operator total training time {t_train:.0f}s (FrFNO {tA:.0f}s + Ensemble x{K_ENS} {tBtot:.0f}s)')

    rows = []
    for (Nx, Nt, Nref) in RES:
        log(f'\n======== Nx={Nx} ({Nx+1}²) Nt={Nt} MC reference N={Nref} ========')
        tpb = time.time(); AD, SD, PT = build_prop_table(Nx, Nt=400); log(f'  propagator table (Nt=400, consistent with training) {time.time()-tpb:.0f}s')
        ...
        for i in range(NTE):
            ...
            # Solver result pool: reference (with nested QMC/GP) + gPC + SC unique nodes
            ...
            # GP (train on first 25 reference points, infer M_INF random samples)
            ng = 25
            ...
            mk = pEns.mean(1)                 # (K,D) member mean
            ale = pEns.var(1).mean(0)         # aleatoric: mean of within-member sample variance
            epi = mk.var(0)                    # epistemic: across-member variance
    ...
    for Nx, rr in rows:
        for name, mi, pct in stat:
            ...
            log(line)
    log(f'\n[One-time training cost shared across three resolutions] FrFNO {tA:.0f}s, DeepEnsemble({K_ENS} nets) {tBtot:.0f}s, total {t_train:.0f}s')
    np.savez(os.path.join(ROOT, 'uq_compare_metrics.npz'),
